...
assertIncludes(readme, '自动同步源支持常见订阅文本解析与节点回写', 'README sync docs');
...
assertIncludes(readme, 'docs/单用户托管模式需求.md', 'README single-user requirement entry');
assertIncludes(readme, 'docs/节点管理与订阅使用说明.md', 'README single-user usage entry');
assertIncludes(readme, 'docs/部署指南.md', 'README deploy guide entry');
assertIncludes(readme, 'docs/单用户主链路速查.md', 'README quick flow entry');
assertIncludes(readme, 'docs/单用户术语速查.md', 'README terminology quick reference entry');
assertIncludes(readme, 'docs/自动同步源状态与错误说明.md', 'README sync diagnostics entry');
assertIncludes(readme, '仓库内部参考', 'README internal docs demotion note');

const deployGuide = readFileSync('docs/部署指南.md', 'utf8');
assertIncludes(deployGuide, '单人使用最小路径', 'deploy guide single-user minimal path');
...
assertIncludes(deployGuide, 'docs/高级部署与数据维护.md', 'deploy guide advanced ops entry');

const advancedDeployGuide = readFileSync('docs/高级部署与数据维护.md', 'utf8');
assertIncludes(advancedDeployGuide, 'GitHub Actions', 'advanced deploy guide github actions docs');
assertIncludes(advancedDeployGuide, 'package-lock.json', 'advanced deploy guide lockfile docs');
assertIncludes(advancedDeployGuide, 'npm ci', 'advanced deploy guide npm ci docs');
assertIncludes(advancedDeployGuide, 'workflow_dispatch', 'advanced deploy guide workflow dispatch docs');
assertIncludes(advancedDeployGuide, 'git_ref', 'advanced deploy guide git ref docs');
assertIncludes(advancedDeployGuide, 'release/*', 'advanced deploy guide release branch docs');
assertIncludes(advancedDeployGuide, 'FAILURE_WEBHOOK_URL', 'advanced deploy guide failure webhook docs');
assertIncludes(advancedDeployGuide, 'wrangler d1 export subforge --remote', 'advanced deploy guide d1 export docs');
assertIncludes(advancedDeployGuide, 'wrangler d1 execute DB --remote', 'advanced deploy guide d1 restore docs');
assertIncludes(advancedDeployGuide, 'npm run backup:d1:decrypt', 'advanced deploy guide backup decrypt docs');
assertIncludes(advancedDeployGuide, 'D1_BACKUP_ARCHIVE_PASSPHRASE', 'advanced deploy guide backup passphrase docs');
assertIncludes(advancedDeployGuide, 'D1_BACKUP_ARCHIVE_S3_URI', 'advanced deploy guide backup object storage uri docs');
assertIncludes(advancedDeployGuide, 'D1_BACKUP_ARCHIVE_ENDPOINT_URL', 'advanced deploy guide backup endpoint docs');
assertIncludes(advancedDeployGuide, 'D1_BACKUP_ARCHIVE_AWS_REGION', 'advanced deploy guide backup region docs');
assertIncludes(advancedDeployGuide, 'D1_BACKUP_ARCHIVE_SSE', 'advanced deploy guide backup sse docs');
assertIncludes(advancedDeployGuide, 'D1_BACKUP_ARCHIVE_KMS_KEY_ID', 'advanced deploy guide backup kms docs');
assertIncludes(advancedDeployGuide, 'D1_BACKUP_ARCHIVE_STORAGE_CLASS', 'advanced deploy guide backup storage class docs');
assertIncludes(advancedDeployGuide, 'D1_BACKUP_ARCHIVE_AWS_ACCESS_KEY_ID', 'advanced deploy guide backup access key docs');
assertIncludes(advancedDeployGuide, 'aws s3 cp', 'advanced deploy guide backup object storage command docs');
assertIncludes(advancedDeployGuide, 'bucket lifecycle', 'advanced deploy guide backup lifecycle docs');
assertIncludes(advancedDeployGuide, '.github/workflows/d1-backup.yml', 'advanced deploy guide d1 backup workflow docs');
assertIncludes(advancedDeployGuide, '03:15 UTC', 'advanced deploy guide d1 backup schedule docs');
assertIncludes(advancedDeployGuide, '.sha256', 'advanced deploy guide backup checksum docs');
assertIncludes(advancedDeployGuide, '参数是数据库名，不是绑定名', 'advanced deploy guide d1 export name note');
assertIncludes(advancedDeployGuide, '空库 / 新库', 'advanced deploy guide empty db restore note');
assertIncludes(advancedDeployGuide, '没有自动 down migration', 'advanced deploy guide rollback boundary docs');
...
assertIncludes(docsIndex, '单人使用只看这几篇', 'docs index single-user headline');
assertIncludes(docsIndex, 'docs/单用户托管模式需求.md', 'docs index single-user requirement entry');
...
assertIncludes(docsIndex, 'docs/单用户主链路速查.md', 'docs index quick flow entry');
assertIncludes(docsIndex, 'docs/单用户术语速查.md', 'docs index terminology quick reference entry');
assertIncludes(docsIndex, 'docs/自动同步源状态与错误说明.md', 'docs index sync diagnostics entry');
