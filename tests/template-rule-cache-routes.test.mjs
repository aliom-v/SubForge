...
test('changing rule source enabled status invalidates all user caches', async () => {
  const { env, token } = await createRouteHarness();
  const { response, payload } = await requestJson(
    'http://127.0.0.1:8787/api/rule-sources/rs_1',
    {
      method: 'PATCH',
      headers: {
        authorization: `Bearer ${token}`,
...
test('changing rule source metadata without toggling enabled does not invalidate caches', async () => {
  const { env, token } = await createRouteHarness();
  const { response, payload } = await requestJson(
    'http://127.0.0.1:8787/api/rule-sources/rs_1',
    {
...
test('updating a rule source rejects invalid formats', async () => {
  const { env, token } = await createRouteHarness();
  const { response, payload } = await requestJson(
    'http://127.0.0.1:8787/api/rule-sources/rs_1',
    {
...
test('updating a rule source rejects invalid urls', async () => {
  const { env, token } = await createRouteHarness();
  const { response, payload } = await requestJson(
    'http://127.0.0.1:8787/api/rule-sources/rs_1',
    {
...
test('updating a missing rule source returns 404', async () => {
  const { env, token } = await createRouteHarness();
  const { response, payload } = await requestJson(
    'http://127.0.0.1:8787/api/rule-sources/rs_missing',
    {
      method: 'PATCH',
      headers: {
        authorization: `Bearer ${token}`,
  ...
  assert.equal(payload.ok, false);
  assert.equal(payload.error.code, 'NOT_FOUND');
  assert.equal(payload.error.message, 'rule source not found');
  assert.deepEqual(env.SUB_CACHE.deletedKeys, []);
  assert.equal(env.DB.auditLogs.length, 0);
});

test('deleting a rule source invalidates all user caches', async () => {
  const { env, token } = await createRouteHarness();
  const { response, payload } = await requestJson(
    'http://127.0.0.1:8787/api/rule-sources/rs_1',
    {
...
test('deleting a missing rule source returns 404', async () => {
  const { env, token } = await createRouteHarness();
  const { response, payload } = await requestJson(
    'http://127.0.0.1:8787/api/rule-sources/rs_missing',
    {
